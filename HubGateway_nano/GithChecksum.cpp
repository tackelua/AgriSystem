...
String sha1(String data) {
	Sha1.init();
	Sha1.print(data.c_str());
	uint8_t* hash = Sha1.result();
	String sha1Result;
	for (int i = 0; i < 20; i++) {
		sha1Result += ("0123456789abcdef"[hash[i] >> 4]);
		sha1Result += ("0123456789abcdef"[hash[i] & 0xf]);
	}
	//Db("sha1: ");
	//DB(sha1Result);
	return sha1Result;
}
String gcs_calc(String data) {
	Db("#GCS Data: ");
	DB(data);
	String _sha = sha1(data);
	long  _sum = 0;;
	for (byte i = 0; i < _sha.length(); i++)
	{
		_sum += _sha[i];
	}
	//Db("gcs: ");
